...
def test_human_creation(human_count):
    ...
    assert len(Storage.humans) == human_count

    expected_human_indexes = [0, 1]
    assert [h.index for h in Storage.humans] == expected_human_indexes

    for human in Storage.humans:
...
def test_human_health_creation(mocked_randint):
    ...
    assert Storage.humans[0].health_points == mocked_health_points
